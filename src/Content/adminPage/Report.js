...
const Report = (props) => {
   ...
   let ratingClinic;
   let ratingDoctor;
   if (!isNaN(props.ratingClinic)) {
    ratingClinic = Math.round((props.ratingClinic + Number.EPSILON) * 100) / 100;
   } else {
     ratingClinic = "Нема оцена.";
   }
   if (!isNaN(props.ratingDoctor)) {
    ratingDoctor = Math.round((props.ratingDoctor + Number.EPSILON) * 100) / 100;
   } else {
     ratingDoctor = "Нема оцена.";
   }
    let renderDaniPrihod = null;
    let renderMesecPrihod = null;
    let renderGodinaPrihod = null;
    let renderDaniPregledi = null;
    let renderMesecPregledi = null;
    let renderGodinaPregledi = null;
     if (props.reportDetail == null) {
    ...
    <td> <h4>Просечна оцена клинике:</h4></td>
      ...
      }} className="h4Tittle">Нема одржаних прегледа ни операција.</h4>
        <h4 style={{
        display: show ? 'block' : 'none'
      }} className="h4Tittle">График прихода на дневном, месечном и годишњем нивоу</h4>
        {renderDaniPrihod}
        {renderMesecPrihod}
        {renderGodinaPrihod}
        <h4 style={{
        display: show ? 'block' : 'none'
      }} className="h4Tittle">График прегледа на дневном, месечном и годишњем нивоу</h4>
        {renderDaniPregledi}
        {renderMesecPregledi}
        {renderGodinaPregledi}
      </div>
